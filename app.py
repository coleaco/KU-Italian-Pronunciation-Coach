...
def extract_segmental_features(audio_path: str, words: List[dict]) -> List[PronunciationEvidence]:
...
level = st.radio("Choose your pronunciation focus", ["Beginner", "Intermediate", "Advanced"])

rec = mic_recorder(start_prompt="🎙️ Start recording", stop_prompt="⏹️ Stop recording", format="wav")
...
if rec and rec.get("bytes"):
    ...
    if st.button("🧠 Analyze pronunciation"):
        ...
        try:
            ...
            st.warning("We couldn’t generate feedback this time. Please try again.")
            st.stop()
        ...
        for i, fp in enumerate(feedback.get("focus_points", []), 1):
            start_t = fp["time_range"][0]
            end_t = fp["time_range"][1]
            st.markdown(
                f"**{i}. {fp['word']}** "
